      ...
      {showSearch && items.length > 0 && (
        <div className="relative max-w-md">
          ...
          <input
            ...
            className="w-full rounded-xl border border-border/80 bg-card/80 pl-10 pr-9 py-2.5 text-sm font-medium text-foreground placeholder:text-muted-foreground/70 shadow-sm transition-all focus:border-emerald-500/80 focus:bg-card focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
          />
          {searchQuery && (
            <button
              type="button"
              onClick={() => setSearchQuery('')}
              className="absolute right-3 top-1/2 -translate-y-1/2 rounded-md p-1 text-muted-foreground hover:bg-accent hover:text-foreground transition-colors"
